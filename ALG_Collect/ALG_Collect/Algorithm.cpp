...
void Algorithm::Algorithm_Math::setSPLineXData(array<double>^ SPLineXData)
{
	this->SPLineXData = gcnew array<double>(SPLineXData->Length);
	SPLineXData->CopyTo(this->SPLineXData, 0);
}
...
bool Algorithm::Algorithm_Math::calCubicSplineSolution(std::vector<double>& input_x, std::vector<double>& input_y, CubicEquationSolution ^% cubicCoeffs, _SplineFilterMode filterMode)
{
	...
	*/
	...
	cubicCoeffs = gcnew CubicEquationSolution(count1);
	...
	bool isSucceed = calTridiagonalMatric(m_a, m_b, m_c, m_d, m_part);
...
void Algorithm::Algorithm_Math::cubicSplineITPL(CubicEquationSolution ^% cubicCoeffs, std::vector<double>& input_x, std::vector<double>& output_x, std::vector<double>& output_y)
{
	std::vector<double> vSPLineXData;
	for each(double d in this->SPLineXData) { vSPLineXData.push_back(d); }

	for (int i = 0; i < input_x.size(); i++)
	{
		int index = getITPLIndex(input_x[i], vSPLineXData);
		if (index >= 0)
		{
			double dertx = input_x[i] - vSPLineXData[index];
...
void Algorithm::Algorithm_Math::cubicSplineITPL2(CubicEquationSolution ^% cubicCoeffs, std::vector<double>& input_x, double & x, double & y)
{
	...
	{
		int index = getITPLIndex(x, input_x);
		if (index > 0)
		{
...
bool Algorithm::Algorithm_Math::calTridiagonalMatric(cv::Mat_<double>& input_a, cv::Mat_<double>& input_b, cv::Mat_<double>& input_c, cv::Mat_<double>& input_d, cv::Mat_<double>& output_x)
{
